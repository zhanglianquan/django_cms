...
function show_data(list) {
	...
	$(".btn_edit").click(function() {
		...
		$("#center-column").load("../../static/templates/data_manager/data_add.html?random=" + Math.random());
...
function do_page() {
	$.getJSON(
		"data_list?type=" + get_menu_param("type") + "&page=" + curr_page + "&random=" + Math.random(),
		function(data) {
			show_data(data.res_data.list);
			update_page_nav(data.res_data);
		}
	);
}
...
$(function() {
	...
	if(get_menu_param("page")) {
		curr_page = get_menu_param("page");
		url = "data_list?type=" + get_menu_param("type") + "&page=" + get_menu_param("page") + "&random=" + Math.random();
	}
	else
		url = "data_list?type=" + get_menu_param("type") + "&random=" + Math.random();

	$.getJSON(
		url,
		function(data) {
			if(data.res_data.page_count == 1) {
